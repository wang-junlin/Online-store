...
from DjangoUeditor.models import UEditorField #外置的一个写好了的app，可以直接拿过来用，后面的xadmin应该也是这样（富文本编辑页面）
...
class GoodsCategory(models.Model):
    ...
    code = models.CharField(default="", max_length=30, verbose_name="类别code", help_text="类别code") #编码，可以通过字母英文来搜索
    ...
    add_time = models.DateTimeField(default=datetime.now,verbose_name="添加时间") #所有表都应该要有的
...
class GoodsCategoryBrand(models.Model):
    ...
    #源代码中此处有一句 category = models.ForeignKey(GoodsCategory, related_name='brands', null=True, blank=True, verbose_name="商品类目")
    ...
    add_time = models.DateTimeField(default=datetime.now,verbose_name="添加时间") #添加时候的初始日期时间

    class Meta:
        verbose_name = "品牌"
        verbose_name_plural = verbose_name
        #源码中，此处应该有一句 db_table = "goods_goodsbrand"
...
class Goods(models.Model):
    ...
    #继承商品类别，默认是必须要选择一个的（默认为null = False），就是不能为空，如果手动加进去（null = Ture），那么就可以不选类别
    ...
    #源码中此处应该是goods_desc = UEditorField(verbose_name=u"内容", imagePath="goods/images/", width=1000, height=300,filePath="goods/files/", default='')
    ...
    add_time = models.DateTimeField(default=datetime.now, verbose_name="添加时间") #每个表都应该有这个
...
class GoodsImage(models.Model):
    ...
    goods = models.ForeignKey(Goods, verbose_name="商品", related_name="images") #名字;related翻译是相关名称的意思
    image = models.ImageField(upload_to="", verbose_name="图片", null=True, blank=True) #轮播图存放位置
    image_url = models.CharField(max_length=300, null=True, blank=True, verbose_name="图片url") #不知道干什么的，应该和图片位置相关
    ...
    def __str__(self):
        return self.goods.name
        #在这个类中使用上面Goods类中的name，所以要在这个类中的goods中继承上面的Goods类，这样才能使用那个类的name。
...
class Banner(models.Model):
    ...
    #这个就是单独显示的轮播大图片，这个是不能为空的，默认就是null=false，所以就不用管它；upload_to="banber"，图片存放目录，好像会自动在之前创建的media中放入这个图片。
